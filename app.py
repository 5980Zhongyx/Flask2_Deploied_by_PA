...
def create_app(config_name=None):
    ...
    app.jinja_env.globals["static_file_exists"] = lambda path: os.path.exists(
    ...
    def before_request():
        ...
        if not user_agent or len(user_agent) < 10:
            # Suspicious request — log brief info
            app.logger.warning(
                "Suspicious request from %s: missing or short User-Agent",
                request.remote_addr,
            )
        ...
        for header in suspicious_headers:
            ...
            if value and (
                "<" in value or ">" in value or "javascript:" in value.lower()
            ):
                app.logger.warning(
                    "Potential XSS attempt in %s: %.100s...", header, value
                )
    ...
    def after_request(response):
        if hasattr(g, "start_time"):
            ...
            if duration > 1.0:  # request that is over 1 second
                try:
                    ...
                    ua = "Unknown"
                app.logger.warning(
                    "slow_request duration=%.3fs user_agent=%s", duration, ua
                )
        ...
        csp_parts = [
            "default-src 'self';",
            # allow scripts from our CDN providers
            (
                "script-src 'self' 'unsafe-inline' "
                "https://cdn.jsdelivr.net https://cdnjs.cloudflare.com;"
            ),
            # allow styles including inline and known CDNs
            (
            ...
            # fonts from self, data and common font CDNs
            (
                "font-src 'self' data: "
...
def cleanup_expired_sessions(app):
    ...
    try:
        # Simple cleanup placeholder; replace with robust session GC if needed
...
def setup_logging(app, config_obj=None):
    ...
    if config_obj is not None:
        try:
            ...
            log_level = None
    if not log_level:
        # Fall back to LOG_LEVEL from app.config
