...
# -*- coding: utf-8 -*-
"""
Evaluate recommendation algorithms (User-based, Item-based, Matrix Factorization)
Output JSON report scripts/recommendation_eval.json

Metrics: precision@K (K=5), hit_rate@1 (whether hit single holdout)
"""
import os
import sys
import json
...
def user_based_recs(train_u2i, user_id, top_n=10):
    ...
    items = set()
    for its in train_u2i.values(): items |= its
    items = list(items)
    ...
    for u, vec in vectors.items():
        ...
        for it in train_u2i[u]:
            if it in train_u2i[user_id]: continue
            scores[it] = scores.get(it,0)+sim
    ranked = sorted(scores.items(), key=lambda x:x[1], reverse=True)
    return [fid for fid,_ in ranked[:top_n]]

def mf_recs_train_and_recommend(train_interactions, user_id, top_n=10, factors=10, epochs=10, lr=0.01, reg=0.02):
    if np is None:
        return []
...
def evaluate(app, sample_users=200, k=5):
    with app.app_context():
        interactions = UserFilmInteraction.query.order_by(UserFilmInteraction.created_at).all()
        u2i_all = build_user_items(interactions)
        users = [u for u,its in u2i_all.items() if len(its)>=2]
        if not users:
            print('No enough users for evaluation, output empty metrics')
            return {
                'n_users': 0,
                'k': k,
        ...
        for u in users:
            ...
            if not user_interactions: continue
            # holdout last interaction
            test = user_interactions[-1]
            train = [it for it in interactions if not (it.user_id==u and it.film_id==test.film_id and it.created_at==test.created_at)]
            ...
            def eval_list(lst):
                ...
                prec = hit / k
                return hit, prec
            h, p = eval_list(user_recs); hits['user']+=h; precision['user'].append(p)
            h, p = eval_list(item_recs); hits['item']+=h; precision['item'].append(p)
            h, p = eval_list(mf_recs); hits['mf']+=h; precision['mf'].append(p)
        n = len(users)
        results = {
            'n_users': n,
            'k': k,
            'user_based': {'hit_rate': hits['user']/n, 'precision_at_k': sum(precision['user'])/n},
            'item_based': {'hit_rate': hits['item']/n, 'precision_at_k': sum(precision['item'])/n},
            'matrix_factorization': {'hit_rate': hits['mf']/n, 'precision_at_k': sum(precision['mf'])/n},
        }
        return results
