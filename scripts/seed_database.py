...
def create_sample_interactions():
    ...
    test_users = [
        User(
            ...
            password_hash="pbkdf2:sha256:150000$dummy$dummy",
        ),
        User(
    ...
    for movie in classic_movies[:5]:  # 喜欢其中5部
        interaction = UserFilmInteraction(
            user_id=movie_fan.id, film_id=movie.id, liked=True, rating=4
        )
    ...
    for movie in action_movies[:6]:  # 喜欢其中6部
        interaction = UserFilmInteraction(
            user_id=action_lover.id, film_id=movie.id, liked=True, rating=5
        )
        db.session.add(interaction)
    ...
    for movie in shared_movies:
        ...
        for user in [movie_fan, action_lover]:
            interaction = UserFilmInteraction(
                user_id=user.id, film_id=movie.id, liked=True, rating=4
            )
            db.session.add(interaction)
